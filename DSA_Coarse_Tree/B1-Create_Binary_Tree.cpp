...
void levelOrderTraversal(node* root) {
    ...
    q.push(NULL); // Enqueue a marker to indicate the end of each level
    ...
    while (!q.empty()) {
        ...
        if (curr == NULL) {
            // If the marker is encountered, it means the current level is complete
            cout << endl; // Print a new line to separate levels

            if (!q.empty()) {
                // If the queue still has nodes, enqueue another marker for the next level
                q.push(NULL);
            }
        } else {
